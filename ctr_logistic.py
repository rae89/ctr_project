...
beta = .6  # smoothing parameter for adaptive learning rate
L1 = 1.    # L1 regularization, larger value means more regularized
L2 = 1.5    # L2 regularization, larger value means more regularized
...
holdafter = False   # data after date N (exclusive) are used as validation
...
def data(path, D):
    ...
    for t, row in enumerate(DictReader(open(path))):
        ...
        year = int(row['hour'][0:2]) 
        month = int(row['hour'][2:4])
        dw = int(datetime.date(year,month,date).weekday())
...
for e in xrange(epoch):
    ...
    for t, date, dw, ID, x, y in data(train, D):  # data is a generator
...
with open(submission, 'w') as outfile:
    ...
    for t, date, dw, ID, x, y in data(test, D):
        p = learner.predict(x)
        outfile.write('%s,%s\n' % (ID, str(p)))
